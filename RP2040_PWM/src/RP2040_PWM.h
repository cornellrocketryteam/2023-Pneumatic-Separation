...
#warning USING_MBED_RP2040_PWM in RP2040_PWM.h
       ...
       defined(ARDUINO_WIZNET_5100S_EVB_PICO) || defined(ARDUINO_GENERIC_RP2040)) &&                                                                   \
    !defined(ARDUINO_ARCH_MBED)
...
public:
                     ...
                     uint16_t &level, bool phaseCorrect = false)
  {
    ...
    pwm_init(_slice_num, _slice_num, &config, true);
    pwm_set_gpio_level(_pin, level);
  ...
  bool setPWMPushPull_Int(const uint8_t &pinA, const uint8_t &pinB, const float &frequency, const uint32_t &dutycycle)
  {
    ...
    if ((frequency <= ((float)MAX_PWM_FREQUENCY * freq_CPU / 125000000)) && (frequency >= ((float)MIN_PWM_FREQUENCY * freq_CPU / 125000000)))
    {
